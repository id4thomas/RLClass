...
class ACModel():
    def __init__(self,env):
        ...
        self.actor,self.sigma=self.make_actor()
    ...
    def make_critic(self):
        ...
        d1=tf.keras.layers.Dense(128, activation='tanh',kernel_initializer='he_uniform')(in1)
        d2=keras.layers.Dense(128, activation='tanh',kernel_initializer='he_uniform')(d1)
        #critic
    ...
    def make_actor(self):
        ...
        d1=tf.keras.layers.Dense(128, activation='tanh',kernel_initializer='he_uniform')(in1)
        d2=keras.layers.Dense(128, activation='tanh',kernel_initializer='he_uniform')(d1)
        mu=tf.keras.layers.Dense(self.a_size, activation='tanh',kernel_initializer='he_uniform',name="mu")(d2)
        ...
        mu=keras.models.Model(inputs=in1,outputs=mu)
        #logstddev
        sigma=tf.Variable([0.]*self.a_size,trainable=True,shape=[self.a_size])
        return mu,sigma
    
    def get_action(self,state):
        mu=self.actor(state,training=False)
        sigma=self.sigma
    ...
    def apply_grads(self,a_grads,sig_grad,c_grads):
    #def apply_grads(self,grads,trainables):
        grads=a_grads+[sig_grad]+c_grads
        trainables=self.actor.trainable_weights+[self.sigma]+self.critic.trainable_weights
        #print(trainables)
...
class PPO():
    ...
    def calc_grads(self,batch):
        ...
        with tf.GradientTape(persistent=True) as t:
            ...
            t.watch(self.net.sigma)
            t.watch(self.net.critic.trainable_weights)
            #t.watch(self.trainables)
            #print(self.net.get_logp(s,a))
            ratio=tf.exp(self.net.get_logp(s,a) - tf.stop_gradient(self.old_net.get_logp(s,a)))
    ...
    def run(self):
        ...
        while cur_iter<self.num_iters:
            ...
            self.old_net.sigma=self.net.sigma
            #print(self.old_net.sigma)
            #get trajectories
            cur_steps=0
    ...
    def train(self,T):
        ...
        for ep in range(self.num_epochs):
            ...
            p_loss=0
            if batch_iters==0:
                a_grads,sig_grad,c_grads,p_loss,v_loss=self.calc_grads(vals)
                #grads,p_loss,v_loss=self.calc_grads(vals)
                self.net.apply_grads(a_grads,sig_grad,c_grads)
                ...
                for i in range(batch_iters):
                    ...
                    a_grads,sig_grad,c_grads,a_loss,c_loss=self.calc_grads(batch)
